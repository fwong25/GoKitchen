<template>
    <subpage-template>
        <form>
            <div class="input-group mb-2">
                ...
                <div class="input-group-append text-info">
                    <span class="input-group-text bg-white py-0">
                        <button type="submit" class="btn btn-sm text-info" @click.stop.prevent="updateRecipe()">
                            <h6>
                ...
                <div class="input-group-append text-info">
                    <span class="input-group-text bg-white py-0">
                        <button type="submit" class="btn btn-sm text-info"  @click.stop.prevent="cancelEditRecipe()">
            ...
            <!-- <ul v-if="this.recipe.ParentRecipeID != 0" class="nav nav-pills"> -->
            ...
            <br>
            <div v-for="ingredient_group in recipe.Ingredients">
...
import { host_info } from './../main.js'
    ...
    methods: {
                ...
                response.json().then((data) => {
                    ...
                    if (this.recipe.Ingredients != null) {
                        for (let i = 0; i < this.recipe.Ingredients.length; ++i) {
                            this.recipe.Ingredients[i]["id"] = i;
                            this.recipe.Ingredients[i].new_ingredient_name = '',
                            this.recipe.Ingredients[i].new_amount = '',
                            this.recipe.Ingredients[i].new_amount_unit = ''

                            this.recipe.Ingredients[i].edit_group_name = ''

                            if (this.recipe.Ingredients[i].Ingredients != null) {
                                for (let j = 0; j < this.recipe.Ingredients[i].Ingredients.length; ++j) {
                                    this.recipe.Ingredients[i].Ingredients[j]["id"] = j+1;
                                }
                            }
                        }
                        this.ingredient_category_id = this.recipe.Ingredients.length
                    }  
                    else {
                    ...
                    this.original_recipe = this.recipe

                    console.log(this.recipe.Ingredients)

                    if(this.recipe.IsRescaled) {
                        this.rescaleRecipe();
                    } else {
                        this.modifyRecipe();
                    }
        ...
        updateRecipe() {
            ...
            this.recipe.ParentRecipeID = parseInt(this.recipe.ParentRecipeID);

            var ingredients = []
            if (this.recipe.Ingredients != null)
                for(let i = 0; i < this.recipe.Ingredients.length; ++i) {
                    var ingredient = {
                        categoryName: this.recipe.Ingredients[i].CategoryName,
                        ingredients: []
                    }
                    if (this.recipe.Ingredients[i].Ingredients != null) {
                        for(let j = 0; j < this.recipe.Ingredients[i].Ingredients.length; ++j) {
                            var ingredient_entry = {
                                ingredientName: this.recipe.Ingredients[i].Ingredients[j].IngredientName,
                                amount: this.recipe.Ingredients[i].Ingredients[j].Amount,
                                amountUnit: this.recipe.Ingredients[i].Ingredients[j].AmountUnit
                            }
            ...
            console.log(this.recipe.RecipeRescaleInfo)
            console.log(JSON.stringify({
                        recipeID: parseInt(this.$route.query.recipe_id),
            ...
            fetch('http://' + host_info.backend_host + '/update_recipe',
                {
                    method: 'PUT',
                    mode: 'cors',
                    headers: {
        ...
        addIngredient(ingredient_category_id) {
            ...
            this.ingredient_order_id = this.recipe.Ingredients[ingredient_category_id].Ingredients.length + 1
            var ingredient = {
                id: this.ingredient_order_id,
                IngredientName: this.recipe.Ingredients[ingredient_category_id].new_ingredient_name,
                Amount: this.recipe.Ingredients[ingredient_category_id].new_amount,
                AmountUnit: this.recipe.Ingredients[ingredient_category_id].new_amount_unit
            }
            this.recipe.Ingredients[ingredient_category_id].Ingredients.push(ingredient)

            this.recipe.Ingredients[ingredient_category_id].new_ingredient_name = ''
            this.recipe.Ingredients[ingredient_category_id].new_amount = ''
            this.recipe.Ingredients[ingredient_category_id].new_amount_unit = ''
        },
        editIngredient(edit_ingredient_order_id, edit_ingredient_category_id) {
            ...
            this.edit_ingredient_name = this.recipe.Ingredients[edit_ingredient_category_id].Ingredients[edit_ingredient_order_id-1].IngredientName
            this.edit_amount = this.recipe.Ingredients[edit_ingredient_category_id].Ingredients[edit_ingredient_order_id-1].Amount
            this.edit_amount_unit = this.recipe.Ingredients[edit_ingredient_category_id].Ingredients[edit_ingredient_order_id-1].AmountUnit
        },
        deleteIngredient(ingredient_id, category_id) {
            ingredient_id -= 1
            this.recipe.Ingredients[category_id].Ingredients.splice(ingredient_id, 1)
            for(let i = ingredient_id; i < this.recipe.Ingredients[category_id].Ingredients.length; ++i) {
                this.recipe.Ingredients[category_id].Ingredients[i].id -= 1
            }
        },
        confirmEditIngredient() {
            this.recipe.Ingredients[this.edit_ingredient_category_id].Ingredients[this.edit_ingredient_order_id-1].IngredientName = this.edit_ingredient_name
            this.recipe.Ingredients[this.edit_ingredient_category_id].Ingredients[this.edit_ingredient_order_id-1].Amount = this.edit_amount
            this.recipe.Ingredients[this.edit_ingredient_category_id].Ingredients[this.edit_ingredient_order_id-1].AmountUnit = this.edit_amount_unit
        ...
        rescaleRecipe() {
            ...
            if(this.subrecipe_mode != "rescale") {
                ...
                for(let i = 0; i < this.recipe.Ingredients.length; ++i) {
                    for(let j = 0; j < this.recipe.Ingredients[i].Ingredients.length; ++j) {
                        this.recipe.Ingredients[i].Ingredients[j].NewAmount = this.recipe.Ingredients[i].Ingredients[j].Amount
                    }
                }
                ...
                if(isNaN(this.recipe.Ingredients[0].Ingredients[0].Amount))
                    this.recipe.RecipeRescaleInfo.NewAmount = -1
                else
                    this.recipe.RecipeRescaleInfo.NewAmount = parseInt(this.recipe.Ingredients[0].Ingredients[0].Amount)
        ...
        selectRescaleIngredient(ingredient_id, category_id) {
            if(!isNaN(this.recipe.Ingredients[category_id].Ingredients[ingredient_id-1].NewAmount)) {
                this.rescale_ing_id = ingredient_id-1
                this.rescale_cat_id = category_id
                this.rescale_amount = this.recipe.Ingredients[category_id].Ingredients[ingredient_id-1].NewAmount
            }
        },
        confirmRescaleIngredient() {
            // this.recipe.RecipeIngredients[this.rescale_ing_id].NewAmount = this.rescale_amount

            var scale = this.rescale_amount / this.recipe.Ingredients[this.rescale_cat_id].Ingredients[this.rescale_ing_id].Amount
            for(let i = 0; i < this.recipe.Ingredients.length; ++i) {
                for(let j = 0; j < this.recipe.Ingredients[i].Ingredients.length; ++j) {
                    this.recipe.Ingredients[i].Ingredients[j].NewAmount *= scale
                }
            }

            this.recipe.RecipeRescaleInfo.RecipeID = this.recipe.RecipeID
            this.recipe.RecipeRescaleInfo.IngredientID = this.recipe.Ingredients[this.rescale_cat_id].Ingredients[this.rescale_ing_id].RecipeIngredientID
        ...
        resetRecipe() {
            this.recipe.Title = this.original_recipe.Title
            this.recipe.Steps = this.original_recipe.Steps
            this.recipe.ParentRecipeID = this.original_recipe.ParentRecipeID
            this.recipe.IsRescaled = this.original_recipe.IsRescaled
            this.recipe.Ingredients = this.original_recipe.Ingredients
            this.recipe.RecipeRescaleInfo = this.original_recipe.RecipeRescaleInfo
        ...
        insertIngredientCategory(category_name) {
            ...
            this.recipe.Ingredients.push(ingredient_category)
        ...
        deleteIngredientGroup(ingredient_category_id) {
            this.recipe.Ingredients.splice(ingredient_category_id, 1)
            for(let i = ingredient_category_id; i < this.recipe.Ingredients.length; ++i) {
                this.recipe.Ingredients[i].id -= 1
            }
        },
        editIngredientGroup(ingredient_category_id) {
            ...
            this.recipe.Ingredients[ingredient_category_id].edit_group_name = this.recipe.Ingredients[ingredient_category_id].CategoryName
        },
        confirmEditIngredientGroup(ingredient_category_id) {
            this.edit_ingredient_group_id = -1
            this.recipe.Ingredients[ingredient_category_id].CategoryName = this.recipe.Ingredients[ingredient_category_id].edit_group_name
        },
